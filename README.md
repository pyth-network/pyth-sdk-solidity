...
Price updates are not submitted on the EVM networks automatically: rather, when a consumer needs to use the value of a price they should first submit the latest Wormhole update for that price to the Pyth contract on the EVM network they are working on. This will make the most recent price update available on-chain for EVM contracts to use. Price Service provides an API to get these messages, Refer to (pyth-evm-js)[https://github.com/pyth-network/pyth-js/tree/main/pyth-evm-js] for more information on how to use it.
